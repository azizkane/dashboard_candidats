...
// Helper local: récupère un token possible (local/session)
...
const AppBarElecteur = ({ title }: AppBarElecteurProps) => {
  const [open, setOpen] = useState(false);        // panneau notifs
  ...
  // 👉 Une seule requête: /api/notifications ; badge = list.length
  const refresh = async () => {
    if (!token) return;
    try {
      setLoading(true);
  ...
  // fermer panneaux si clic en dehors
  useEffect(() => {
    const onClick = (e: MouseEvent) => {
      const t = e.target as Node;
      // notif
      if (open && !(panelRef.current?.contains(t) || bellRef.current?.contains(t))) {
        setOpen(false);
      }
      // user menu
      if (userOpen && !(userMenuRef.current?.contains(t) || userBtnRef.current?.contains(t))) {
        setUserOpen(false);
      }
    };
  ...
  const handleLogout = async () => {
    try {
      ...
      // même si erreur, on nettoie côté client
    } finally {
      // nettoyage local
    ...
    <header className="fixed top-0 left-64 right-0 h-16 bg-white shadow-md px-6 flex items-center justify-between z-40">
      ...
      <div className="flex items-center gap-4 relative">
        ...
        {userOpen && (
          ...
          >
            <button
              onClick={() => { setUserOpen(false); navigate('/electeur/profil'); }}
